...
// each player.  It is an integer.

// NOTE: get_stats_for_lastX_weeks is one of the main functions.
function get_stats_for_last_x_weeks (field, lastX) {
  return filter_last_x_tournaments(field, lastX);
}

function get_stats_for_last_x_weeks_from_db (lastX) {
...
// NOTE: get_all_stats_for_current_field is one of the main functions.
function get_all_stats_for_current_field () {
  // NOTE: first we get salaries and curr tourn id's
  return dk_salary_AND_currTourn_id()
  .then(res => {
    // NOTE: then we pass the response to the next function to
